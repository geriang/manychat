...
const findEmail = async (chatHistory) => {
    ...
    const lookUpEmailTemplate = `You are tasked to extract and validate a client's email address from a given data source. Are you able to identify client's email address from the following chat history? Is the client's email address in a valid email format?
    Chat History: {chat_history}
    Observation: This is your observation on the task given:`
    ...
    const extractEmailTemplate = `Given the observation, is the client's email address in a valid email format? If it is in a valid format, please extract out the email address by wrapping it with "<" ">". For example,<mary@abc.com>. Otherwise say no email is found.
    Observation: {observation}
